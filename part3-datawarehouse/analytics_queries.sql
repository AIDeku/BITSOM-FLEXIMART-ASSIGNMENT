-- Part 3: Data Warehouse - OLAP Analytics Queries
...
-- This helps in seeing the trend from a high level down to the specifics.
...
SELECT 
    ...
    CONCAT(ROUND(SUM(f.total_amount) * 100.0 / (SELECT SUM(total_amount) FROM fact_sales), 1), '%') AS revenue_percentage
FROM fact_sales f
